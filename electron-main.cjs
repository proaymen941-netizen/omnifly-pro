...
// IPC handler for Native WhatsApp Desktop integration
ipcMain.handle("open-whatsapp-desktop", async (_event, { phone, text }) => {
  try {
    let cleanPhone = (phone || "").replace(/\D/g, "");
    if (cleanPhone.startsWith("0")) cleanPhone = "966" + cleanPhone.slice(1);
    else if (cleanPhone.length === 9) cleanPhone = "966" + cleanPhone;
    const desktopUri = `whatsapp://send?phone=${cleanPhone}&text=${encodeURIComponent(text || "")}`;
    await shell.openExternal(desktopUri);
    return { success: true, uri: desktopUri };
  } catch (err) {
