...
router.get("/licenses/active", (req, res) => {
  ...
  if (!lic) {
    res.json({ active: false, client_name: "غير مرخص / موقوف", expires_at: "", devices_limit: 0 });
    return;
  }
...
router.post("/licenses", (req, res) => {
  ...
  const r = db.prepare("INSERT INTO licenses (license_key, client_name, devices_limit, expires_at, active, status) VALUES (?,?,?,?,1,'active')")
    .run(licenseKey, client_name ?? "عميل جديد", devices_limit ?? 1, expires_at ?? "2027-12-31");
  logAudit(userId, userName, "إصدار ترخيص", `ترخيص لـ ${client_name ?? "عميل جديد"} برقم ${licenseKey} (عدد الأجهزة: ${devices_limit ?? 1})`);
  res.status(201).json({ id: r.lastInsertRowid, license_key: licenseKey, client_name, devices_limit: devices_limit ?? 1, expires_at, active: 1, status: "active" });
});
...
router.patch("/licenses/:id", (req, res) => {
  ...
  const userId = user?.id ?? 1;
  const userName = user?.name ?? "مطور النظام";
  const { status, active, expires_at, expire_date, client_name, devices_limit } = req.body;
  ...
  db.prepare(`
    UPDATE licenses 
    SET client_name=?, devices_limit=?, expires_at=?, active=?, status=?
    WHERE id=?
  `).run(finalClientName, finalDevicesLimit, finalExpiresAt, finalActive, finalStatus, req.params.id);
  ...
  const devices = db.prepare("SELECT * FROM license_devices WHERE license_id=?").all(updated.id);
...
router.post("/licenses/verify", (req, res) => {
  ...
  const lic = db.prepare("SELECT * FROM licenses WHERE license_key=? AND active=1 AND (status IS NULL OR status='active')").get(license_key) as any;
  if (!lic) {
    res.status(400).json({ valid: false, error: "مفتاح الترخيص غير صالح أو معطل / موقوف" });
    return;
  }
