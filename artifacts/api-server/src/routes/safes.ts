...
function logSafeTransaction(params: {
  safe_id: number;
  safe_name: string;
  user_name: string;
  user_id?: number;
  ...
  try {
    const countRow = db.prepare("SELECT COUNT(*) as c FROM safe_transactions").get() as { c: number };
    const trxNum = `TRX-SAF-${String(countRow.c + 1).padStart(4, "0")}`;
    db.prepare(`
    ...
    `).run(
      ...
      params.user_name,
      params.amount,
      params.currency || "ريال",
      params.statement,
      params.reference_number || null,
      params.operation_type
    );
  } catch (err: any) {
    console.error("Error logging safe transaction:", err.message);
  }
}

// Get safe transactions history
router.get("/safes/transactions", (req, res) => {
  const user = requireAuth(req, res);
  if (!user) return;

  try {
    const { safe_id, operation_type } = req.query;
    let query = "SELECT * FROM safe_transactions WHERE 1=1";
    const params: any[] = [];
...
// Create new safe transaction (قبض / صرف / إيداع / سحب / تسوية)
router.post("/safes/transactions", (req, res) => {
  const user = requireAuth(req, res);
  if (!user) return;

  const { safe_id, operation_type, amount, currency, statement, reference_number } = req.body;
  if (!safe_id || !operation_type || !amount || Number(amount) <= 0 || !statement) {
    res.status(400).json({ error: "بيانات الحركة ناقصة (رقم الصندوق، نوع العملية، المبلغ، البيان)" });
    return;
  }

  try {
    const safe = db.prepare("SELECT * FROM safes WHERE id = ?").get(safe_id) as any;
    if (!safe) {
    ...
    if (["expense", "withdrawal", "transfer"].includes(operation_type) && safe.balance < val) {
      res.status(400).json({ error: "رصيد الصندوق غير كافٍ لإجراء هذه العملية!" });
      return;
    }

    if (["receipt", "deposit"].includes(operation_type)) {
      db.prepare("UPDATE safes SET balance = balance + ? WHERE id = ?").run(val, safe.id);
    } else if (["expense", "withdrawal"].includes(operation_type)) {
      db.prepare("UPDATE safes SET balance = balance - ? WHERE id = ?").run(val, safe.id);
    }
    ...
    logSafeTransaction({
      ...
      user_name: user.name,
      amount: val,
      currency: currency || safe.currency || "ريال",
      statement,
      reference_number,
      operation_type
    });

    logAudit(user.id, user.name, `عملية صندوق (${operation_type})`, `${statement} بمبلغ ${val} ${currency || "ريال"}`);
    res.status(201).json({ success: true, new_balance: db.prepare("SELECT balance FROM safes WHERE id = ?").get(safe.id) });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});
