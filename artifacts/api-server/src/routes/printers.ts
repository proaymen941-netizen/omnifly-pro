...
function generateReceiptText(order: any, settings: any, cashierName: string, printerSettings?: any): string {
  ...
  // Left margin in spaces (calibrated for 80mm thermal printers with 8mm recommended margin)
  const leftMarginSpaces = Math.max(1, Math.floor((printerSettings?.leftMargin ?? 8) / 1.5));
...
router.get(["/printers/list", "/printers"], (_req, res) => {
