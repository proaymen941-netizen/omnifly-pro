...
router.post("/travel/passengers", (req, res) => {
  ...
  let finalExpectedExit = expected_exit_date;
  let finalRemainingDays = remaining_days;

  if (travel_date && program_duration_days) {
    const tDate = new Date(travel_date);
    if (!isNaN(tDate.getTime())) {
      const exitDate = new Date(tDate);
      exitDate.setDate(exitDate.getDate() + Number(program_duration_days));
  ...
  const info = stmt.run(
    ...
    visa_type || 'تأشيرة عمره', travel_date || null, program_duration_days ? Number(program_duration_days) : 90,
    finalExpectedExit || null, finalRemainingDays !== undefined ? Number(finalRemainingDays) : null, travel_status || 'داخل مكة'
  );
  ...
  recordAuditLog({
    ...
    details: `إضافة المسافر: ${name_ar} (${name_en}) - نوع التأشيرة: ${visa_type || 'عمرة'} - جواز: ${passport_number || 'بدون'}`,
    newData: newPax
  });
...
router.post("/travel/passengers/bulk-import", (req, res) => {
  ...
  const runImport = db.transaction(() => {
    for (const p of passengers) {
      ...
      const name_ar = p.name_ar || p.name_en;
      const name_en = p.name_en || p.name_ar;
      ...
      let remainingDays = p.remaining_days !== undefined && p.remaining_days !== null ? Number(p.remaining_days) : null;
      ...
      stmt.run(
        p.customer_id || customer_id || null,
        name_ar,
        name_en,
        p.title || 'Mr',
        p.dob || null,
