...
router.post("/orders", (req, res) => {
  try {
    ...
    const { items, paymentMethod, subtotal, discount, tax, total, cashAmount, cardAmount, customerId, userId, note, orderType, tableNumber, safeId, safe_id } = req.body;
    ...
    // Globally sequential and unique invoice number generation to prevent unique constraint failures
    const allOrders = db.prepare("SELECT invoice_number FROM orders").all() as { invoice_number: string }[];
    let maxNum = 0;
    for (const o of allOrders) {
      const matches = String(o.invoice_number).match(/\d+/g);
    ...
    try {
      const r = db.prepare(`
        INSERT INTO orders (
          ...
          customer_id, user_id, note, order_type, 
          table_number, safe_id, created_at, status
        )
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'completed')
      `).run(
        invoiceNumber,
      ...
      console.warn("Order insert with safe_id failed, retrying without safe_id:", insertErr.message);
      const r2 = db.prepare(`
        INSERT INTO orders (
          invoice_number, subtotal, discount, tax, total, 
          payment_method, cash_amount, card_amount, 
    ...
    // Update Safe Balance if cash or card
    try {
      if (paymentMethod === "cash" || paymentMethod === "card") {
        db.prepare("UPDATE safes SET balance = balance + ? WHERE id=?").run(total, effectiveSafeId);
      }
    } catch (safeErr: any) {
      console.warn("Safe balance update notice:", safeErr.message);
    }
    ...
    try {
      ...
      const debitAccount = isCredit ? getCustomerAccountCode(customerId) : "11100"; // AR or Safe cash
      const revenueAccount = "41000"; // Sales revenue

      const lines = [
        { account_code: debitAccount, debit: total, credit: 0, description: `مبيعات الفاتورة رقم ${invoiceNumber}` },
      ...
      if (isCredit && customerId) {
        db.prepare("UPDATE customers SET balance = balance + ? WHERE id=?").run(total, customerId);
      }
