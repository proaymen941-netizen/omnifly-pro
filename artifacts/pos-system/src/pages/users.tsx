...
type FormData = {
  username: string;
  name: string;
  role: "admin" | "general_manager" | "accountant" | "cashier" | "storekeeper" | "purchasing" | "sales" | "hr" | "gds_spec";
  password: string;
  active: boolean;
...
const ROLES = [
  { value: "admin", label: "مدير عام الشركة / الفرع (كامل الصلاحيات المطلقة)" },
  { value: "general_manager", label: "مدير عام الشركة (جميع صلاحيات الأنظمة والبرامج)" },
  { value: "sales", label: "موظف مبيعات وحجوزات سياحية" },
  { value: "accountant", label: "محاسب الحسابات والعمولات السياحية" },
  { value: "purchasing", label: "مسؤول مشتريات الخدمات والموردين" },
  { value: "hr", label: "مسؤول شؤون الموظفين والوكلاء" },
  { value: "gds_spec", label: "أخصائي أنظمة GDS وإصدار التذاكر" },
  { value: "storekeeper", label: "مسؤول عهدة المستندات ووثائق السفر" },
  { value: "cashier", label: "مسؤول الصندوق والخزينة والسندات" },
];

const isAdminRole = (r?: string) => r === "admin" || r === "general_manager" || r === "developer" || r === "مدير" || r === "مدير عام" || r === "مدير عام الشركة";
...
export default function Users() {
  ...
  const openEdit = (u: any) => {
    setEditing(u);
    const isUserAdmin = isAdminRole(u.role) || u.username === "admin" || u.username === "developer";
    setForm({
      username: u.username,
      name: u.name,
      role: u.role || "admin",
      ...
      perm_cancel_invoice: isUserAdmin ? true : (u.perm_cancel_invoice !== undefined ? Boolean(u.perm_cancel_invoice) : true),
  ...
  const handleRoleChange = (r: any) => {
    const isTargetAdmin = isAdminRole(r);
    let perms = {
      perm_create_invoice: isTargetAdmin,
      perm_edit_invoice: isTargetAdmin,
      perm_cancel_invoice: isTargetAdmin,
      perm_return: isTargetAdmin,
      perm_view_prices: isTargetAdmin,
      perm_view_profits: isTargetAdmin,
      perm_edit_stock: isTargetAdmin,
      perm_stocktake: isTargetAdmin,
      perm_edit_entries: isTargetAdmin,
      perm_close_periods: isTargetAdmin,
      perm_view_salaries: isTargetAdmin
    };
    ...
    setForm({
      ...form,
      role: r,
      can_discount: isTargetAdmin || r === "accountant",
      ...perms
    });
  };
  ...
  const handleSave = () => {
    ...
    const payload: any = {
      ...
      role: form.role,
      active: form.active,
      can_discount: isFormAdmin || form.role === "accountant" ? true : form.can_discount,
      employee_id: form.employee_id ? Number(form.employee_id) : null,
      branch_id: form.branch_id ? Number(form.branch_id) : null,
      perm_create_invoice: isFormAdmin ? true : form.perm_create_invoice,
      perm_edit_invoice: isFormAdmin ? true : form.perm_edit_invoice,
      perm_cancel_invoice: isFormAdmin ? true : form.perm_cancel_invoice,
      perm_return: isFormAdmin ? true : form.perm_return,
      perm_view_prices: isFormAdmin ? true : form.perm_view_prices,
      perm_view_profits: isFormAdmin ? true : form.perm_view_profits,
      perm_edit_stock: isFormAdmin ? true : form.perm_edit_stock,
      perm_stocktake: isFormAdmin ? true : form.perm_stocktake,
      perm_edit_entries: isFormAdmin ? true : form.perm_edit_entries,
      perm_close_periods: isFormAdmin ? true : form.perm_close_periods,
      perm_view_salaries: isFormAdmin ? true : form.perm_view_salaries
    };
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {users.map((u: any, idx: number) => {
                  ...
                  return (
                    <tr key={`user-${u.id || u.username || idx}`} className="hover:bg-muted/30 transition-colors">
        ...
        <DialogContent dir="rtl" className="max-w-2xl overflow-y-auto max-h-[90vh]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
            {isAdminRole(form.role) && (
              <div className="bg-emerald-50 border border-emerald-300 rounded-lg p-3 text-xs flex gap-2 text-emerald-900 col-span-1 md:col-span-2">
                <Shield className="w-4 h-4 text-emerald-700 shrink-0 mt-0.5" />
                <div>
                  <p className="font-bold">حساب مدير عام الشركة / الإدارة العليا (كامل الصلاحيات المطلقة)</p>
                  <p className="text-[11px] text-emerald-800 mt-0.5">
                    تم منح هذا الحساب تلقائياً كافة صلاحيات الوصول والإدارة الشاملة لجميع أنظمة وبرامج المؤسسة (المبيعات، الحسابات والقيود، المشتريات، الموارد البشرية، وإدارة النظام وأمان البيانات).
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                >
                  {ROLES.map((r, rIdx) => (
                    <option key={`role-opt-${r.value || rIdx}`} value={r.value}>{r.label}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  {employees.map((e: any, eIdx: number) => (
                    <option key={`emp-opt-${e.id || eIdx}`} value={e.id}>{e.name} ({e.position})</option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  {branches.map((b: any, bIdx: number) => (
                    <option key={`branch-opt-${b.id || bIdx}`} value={b.id}>{b.name}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 space-y-3">
                ...
                <div className="flex items-center justify-between border-t pt-2">
                  ...
                  <Switch
                    checked={isAdminRole(form.role) || form.role === "accountant" ? true : form.can_discount}
                    disabled={isAdminRole(form.role) || form.role === "accountant"}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b pb-1">
                ...
                <span className="text-[10px] text-blue-700 bg-blue-50 px-2 py-0.5 rounded font-bold">
                  {isAdminRole(form.role) ? "مفعلة بالكامل كمدير عام" : "مخصص للمستخدم"}
                </span>
              </div>

              <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
                {/* Section 1: Sales & Travel Bookings */}
                <div className="space-y-2 border-b pb-2">
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">المبيعات والحجوزات السياحية</h4>
                  
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">إصدار الفواتير وحجوزات السفر والطيران</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_create_invoice} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_create_invoice: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">تعديل الفواتير والحجوزات السياحية</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_edit_invoice} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_edit_invoice: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">إلغاء وحذف الفواتير والتذاكر</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_cancel_invoice} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_cancel_invoice: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">معالجة استرجاع التذاكر والمرتجعات</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_return} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_return: v })} />
                  </div>
                </div>

                {/* Section 2: Financial Views & Commissions */}
                <div className="space-y-2 border-b pb-2">
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">الأسعار والعمولات والربحية</h4>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">عرض أسعار التكلفة والتذاكر الموردة</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_view_prices} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_view_prices: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">عرض أرباح الحجوزات والعمولات الحساسة</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_view_profits} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_view_profits: v })} />
                  </div>
                </div>

                {/* Section 3: Travel Documents & Passports */}
                <div className="space-y-2 border-b pb-2">
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">المستندات وعهد وثائق السفر</h4>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">تعديل وخدمة عهد المستندات والتأشيرات</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_edit_stock} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_edit_stock: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">القيام بجرد وتدقيق وثائق السفر</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_stocktake} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_stocktake: v })} />
                  </div>
                </div>

                {/* Section 4: Accounting & BSP */}
                <div className="space-y-2 border-b pb-2">
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">المحاسبة والقيود والعمولات</h4>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">إنشاء وتعديل قيود الموردين والعملاء</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_edit_entries} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_edit_entries: v })} />
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">إغلاق وتقفيل تسويات الـ BSP والفترات</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_close_periods} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_close_periods: v })} />
                  </div>
                </div>

                {/* Section 5: HR & Agents */}
                <div className="space-y-2">
                  <h4 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">الموارد البشرية والوكلاء</h4>
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-600">الاطلاع على رواتب وعمولات الموظفين والوكلاء</span>
                    <Switch checked={isAdminRole(form.role) ? true : form.perm_view_salaries} disabled={isAdminRole(form.role)} onCheckedChange={v => setForm({ ...form, perm_view_salaries: v })} />
                  </div>
                </div>
              </div>
            </div>
