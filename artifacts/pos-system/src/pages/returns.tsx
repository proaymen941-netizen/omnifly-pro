...
function ViewReturnDialog({ ret, onClose }: { ret: any; onClose: () => void }) {
  ...
  const handlePreview = () => {
    if (printRef.current) {
      setReportHtml(printRef.current.innerHTML);
  ...
  return (
    <Dialog open={!!ret} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="border-b pb-3 flex flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button onClick={handlePreview} variant="outline" size="sm" className="gap-1.5 border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold">
              <Eye className="w-4 h-4 text-indigo-600" /> استعراض قبل الطباعة
            </Button>
            <Button onClick={handlePrint} size="sm" className="gap-2 bg-primary hover:bg-primary/90 font-bold">
        ...
        <DialogFooter className="border-t pt-3 flex justify-between">
          ...
          <Button onClick={handlePreview} variant="outline" className="gap-1.5 border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold">
            <Eye className="w-4 h-4 text-indigo-600" /> استعراض قبل الطباعة
          </Button>
        </DialogFooter>
      </DialogContent>
      ...
      <ReportViewerModal
        ...
        title={`معاينة استعراض سند مردود مبيعات - ${ret.return_number}`}
      />
    </Dialog>
  );
}
