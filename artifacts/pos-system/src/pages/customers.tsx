...
import { Users, Plus, Search, Edit2, Trash2, UserCheck, Eye, Phone, Mail, MapPin, Ticket, Globe, Hotel, Luggage, Printer, FileText, MessageSquare, Building2, Store, Check, ExternalLink, DollarSign, AlertCircle, Lock, ShieldCheck } from "lucide-react";
...
export default function TravelCustomersPage() {
  ...
  // Dynamic fetch of sub-accounts under 11200 (الذمم المدينة)
  const parentCode = "11200";
                      ...
                      return (
                        <tr key={`cust-${c.id || idx}`} className="border-b hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-3 font-mono font-bold">
                            {c.affiliation_type === 'agency' ? (
                              <span className="text-slate-400 cursor-help" title="هذا العميل تابع لمكتب وسيط، رصيده المالي مستقل هو صفر، ويتم احتساب مبيعاته على حساب المكتب">
                                0.00 ريال <span className="text-[10px] block font-normal text-amber-600">تابع لمكتب</span>
                              </span>
                            ) : (
            ...
            >
              ...
              <div className="space-y-3">
                <label className="text-xs font-extrabold text-slate-700 flex items-center gap-1">
                  ...
                  <span>تبعية العميل، وجهة التعامل (المكتب / الوكالة التابع لها) *</span>
                </label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {/* Option 1: Direct Customer */}
                  <div
                    onClick={() => setForm(f => ({ ...f, affiliation_type: 'direct', office_id: "", office_name: "المكتب الرئيسي - المركز الرئيسي", account_code: "" }))}
                  ...
                  >
                    ...
                    <div className="flex flex-col text-right">
                      <span className="text-xs font-extrabold text-slate-900">🏢 تابع للمكتب مباشرة (عميل مباشر)</span>
                  ...
                  {/* Option 2: Agency Customer */}
                  <div
                    onClick={() => setForm(f => ({ ...f, affiliation_type: 'agency', account_code: "" }))}
                    className={`relative flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${
                  ...
                  >
                    ...
                    <div className="flex flex-col text-right">
                      <span className="text-xs font-extrabold text-slate-900">🏬 تابع لمكتب / وكيل وسيط</span>
                      <span className="text-[10px] text-slate-500 mt-1 leading-relaxed">
                        العميل محول أو مسجل عن طريق مكتب سفر أو وكيل فرعي ووسيط
                      </span>
                    </div>
                  </div>
                </div>

                {/* Sub-selection depending on affiliation */}
                {form.affiliation_type === 'direct' ? (
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">اختر فرع / مقر المكتب الرئيسي *</label>
                    <select
                      value={form.office_name || "المكتب الرئيسي - المركز الرئيسي (صنعاء / الرياض) - هاتف: +967 1 200300"}
                    ...
                    >
                      <option value="المكتب الرئيسي - المركز الرئيسي (صنعاء / الرياض) - هاتف: +967 1 200300">
                        🏢 المكتب الرئيسي - المركز الرئيسي (صنعاء / الرياض) - هاتف: +967 1 200300
                      </option>
                      <option value="فرع عدن - شارع تسعين">🏢 فرع عدن - شارع تسعين - هاتف: +967 2 300400</option>
                      <option value="فرع الرياض - حي الملز">🏢 فرع الرياض - حي الملز - هاتف: +966 11 400500</option>
                    </select>
                  </div>
                ) : (
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-white px-3 py-2 text-xs font-bold text-slate-900"
                    >
                      <option value="" disabled>-- اختر المكتب / الوكيل الوسيط --</option>
                      {offices.map((o: any) => (
                ...
                {form.affiliation_type === 'agency' ? (
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                        ...
                        <span>حساب المكتب الوسيط المعتمد في شجرة الحسابات:</span>
                      </label>
                    ...
                    <div className="p-3 bg-purple-50/80 border border-purple-200 rounded-xl flex items-center justify-between">
                      <div>
                        <div className="text-xs font-black text-purple-950">
                          {form.office_name ? `حساب: ${form.office_name}` : "يرجى تحديد المكتب الوسيط من القائمة أعلاه"}
                        </div>
                        <div className="text-[11px] text-purple-700 font-mono mt-0.5">
                          دليل الحسابات &gt; الأصول المتداولة (الذمم المدينة - مكاتب ووكلاء وشركاء) &gt; {form.account_code || "11200"}
                        </div>
                      </div>
                    ...
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs space-y-1.5 text-amber-950">
                      <div className="font-bold text-amber-900 flex items-center gap-1.5">
                        ...
                        <span>قاعدة الأمان والنزاهة المحاسبية (حظر فتح حساب مستقل):</span>
                      </div>
                      <p className="text-amber-800 text-[11px] leading-relaxed">
                        نظراً لأن هذا العميل يتبع مكتباً وكيلاً وسيطاً، <strong>يمنع النظام منعاً باتاً إنشاء أي حساب منفصل أو كشف حساب مستقل لهذا العميل في شجرة الحسابات</strong>. 
                        وتُرحل وتُضاف كافة العمليات والتذاكر والمعاملات والتأثيرات المالية مباشرة إلى <strong>كشف حساب المكتب الوسيط ({form.office_name || 'المكتب المختار'})</strong> مع ذكر اسم هذا العميل صراحة في بيانات وسطور كل قيد وفاتورة.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700 flex items-center gap-1">
                      ...
                      <span>رابط الحساب بدليل الحسابات (شجرة الحسابات) للعميل المباشر *</span>
                    </label>
                    <select
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-white px-3 py-2 text-xs font-bold text-slate-900"
                    >
                      <option value="">✨ إنشاء حساب جديد ومستقل للعميل تلقائياً (الذمم المدينة 11200)</option>
                      {subAccounts.map((acc: any) => (
                        <option key={acc.code} value={acc.code}>
                          {acc.code} - {acc.name} (ذمم مدينة)
                        </option>
                      ))}
                    </select>
                    <p className="text-[11px] text-muted-foreground">
                      عميل مباشر للمكتب: سيتم إنشاء حساب مالي مستقل وكشف حساب خاص به ضمن دليل الحسابات (11200).
                    </p>
                  </div>
                )}
              </div>
          ...
          <DialogContent className="max-w-md">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">نوع الجهة / المكتب *</label>
                  <select
                  ...
                  >
                    <option value="partner_agency">وكيل وسيط شريك</option>
                    <option value="branch">فرع تابع لمكتب</option>
                    <option value="b2b_office">مكتب خدمات تأشيرات</option>
              ...
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-950 flex items-start gap-2.5">
                ...
                <div className="leading-relaxed">
                  ...
                  بمجرد إضافة هذا المكتب، سيتم تلقائياً فتح حساب مالي له في شجرة الحسابات (الذمم المدينة - مكاتب ووكلاء 11200). وستُرحل كافة المعاملات والتذاكر والتأشيرات لجميع العملاء التابعين له إلى كشف حساب هذا المكتب مباشرة دون إنشاء حسابات مستقلة للعملاء التابعين.
                </div>
              </div>
                ...
                {profileTab === "statement" && (
                  <div className="space-y-4">
                    ...
                    <div className="p-4 border rounded-lg bg-slate-50 space-y-3">
                      {profileData.summary.isAgencyAffiliated ? (
                        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                          <div className="flex items-center gap-2 text-amber-800 font-bold mb-1">
                            <AlertCircle className="w-5 h-5 text-amber-600" />
                            <span>تنبيه مالي: عميل تابع لمكتب وسيط</span>
                          </div>
                          <p className="text-xs text-amber-700 leading-relaxed">
                            هذا العميل يتبع محاسبياً لمكتب <strong>({profileData.customer.office_name})</strong>. 
                            لا يتم إنشاء حساب مالي مستقل له، وكافة العمليات تظهر في <strong>كشف حساب المكتب</strong>. 
                            الرصيد المعروض هنا هو "صفر" لأن الذمة المالية تقع على عاتق المكتب وليس العميل.
                          </p>
                        </div>
                      ) : (
