...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [devUser, setDevUser] = useState("developer");
  ...
  const handleGoToLogin = () => {
    ...
    window.location.href = "/login?role=developer&blocked=true";
  };
  ...
  const handleQuickDevLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setDevLoading(true);
    setDevError(null);
    try {
      const res = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({ username: devUser.trim(), password: devPass }),
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || data.error || "بيانات دخول المطور غير صحيحة");
      }
      if (data.user?.role !== "developer" && data.user?.username?.toLowerCase() !== "developer") {
        throw new Error("هذا الحساب ليس حساب مطور النظام المصرح له");
      }
      ...
      setDevError(err.message || "فشل تسجيل الدخول للمطور");
    } finally {
      setDevLoading(false);
    }
  };
    ...
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden" dir="rtl">
      ...
      {licenseBlockedReason && !isDeveloper && (
        <div className="fixed inset-0 z-[99999] bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-slate-900 border-2 border-red-500/50 text-white rounded-3xl p-6 sm:p-8 max-w-lg w-full text-center space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-300 dir-rtl max-h-[95vh] overflow-y-auto custom-scrollbar">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-red-500/20 text-red-500 border-2 border-red-500/40 rounded-2xl flex items-center justify-center mx-auto shadow-inner">
              <Lock className="w-8 h-8 sm:w-10 sm:h-10 animate-bounce" />
            </div>

            <div className="space-y-2">
              <span className="px-3 py-1 bg-red-500/20 text-red-400 border border-red-500/30 text-[11px] font-black rounded-full uppercase tracking-widest inline-block">
                توقف النظام بالكامل 🔒
              </span>
              <h2 className="text-xl sm:text-2xl font-black text-white pt-1">تم تجميد وإيقاف عمل النظام</h2>
              <p className="text-xs sm:text-sm text-slate-200 bg-slate-800/90 p-3.5 rounded-xl border border-slate-700 leading-relaxed font-semibold">
                {licenseBlockedReason}
              </p>
            </div>
            ...
            {deviceHwid && (
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-3 flex items-center justify-between gap-2 text-right">
                <div className="space-y-0.5">
                  <div className="text-[10px] text-slate-400 flex items-center gap-1 font-bold">
                    <Fingerprint className="w-3.5 h-3.5 text-amber-400" />
                    بصمة هذا الجهاز للترخيص:
                  </div>
                  <div className="font-mono text-xs font-black text-amber-300 dir-ltr">{deviceHwid}</div>
                </div>
                <button
                  type="button"
                  onClick={handleCopyHwid}
                  className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-bold flex items-center gap-1 border border-slate-700 transition-colors"
            ...
            <div className="space-y-2.5 pt-1">
              <button
                ...
                className="w-full py-3 px-4 bg-gradient-to-r from-blue-700 to-indigo-800 hover:from-blue-600 hover:to-indigo-700 text-white font-black text-sm rounded-xl shadow-lg border border-blue-500/50 flex items-center justify-center gap-2 transition-all hover:scale-[1.01]"
              >
                <LogIn className="w-4 h-4" />
                <span>الانتقال إلى واجهة تسجيل الدخول (دخول المطور)</span>
              </button>

              <button
                type="button"
                onClick={() => setDevLoginOpen(!devLoginOpen)}
                className="w-full py-2 px-3 bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold text-xs rounded-xl border border-amber-500/30 flex items-center justify-center gap-1.5 transition-colors"
              >
                <Key className="w-3.5 h-3.5" />
                <span>{devLoginOpen ? "إخفاء نموذج دخول المطور السريع" : "تسجيل دخول المطور المباشر لفك التجميد والترخيص ⚡"}</span>
              </button>
            </div>

            {/* Quick Developer Login Form */}
            {devLoginOpen && (
              <form onSubmit={handleQuickDevLogin} className="bg-slate-950/90 border border-amber-500/40 rounded-2xl p-4 text-right space-y-3 animate-in fade-in duration-200">
                <div className="text-xs font-black text-amber-400 flex items-center gap-1.5">
                  <Shield className="w-4 h-4 text-amber-400" />
                  تسجيل دخول مطور النظام الفوري:
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] text-slate-300 font-bold">اسم مستخدم المطور:</label>
                  <input
                    type="text"
                    value={devUser}
                    onChange={(e) => setDevUser(e.target.value)}
                    required
                    className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-3 py-1.5 text-xs font-bold text-right focus:border-amber-400 outline-none"
                    placeholder="developer"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-[11px] text-slate-300 font-bold">كلمة مرور المطور:</label>
                  <input
                    type="password"
                    value={devPass}
                ...
                <button
                  type="submit"
                  disabled={devLoading}
                  className="w-full py-2 bg-amber-600 hover:bg-amber-500 text-slate-950 font-black text-xs rounded-lg shadow-md transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5 mt-1"
                >
                  <Key className="w-3.5 h-3.5" />
                  <span>{devLoading ? "جاري التحقق والاعتماد..." : "فك القفل والانتقال لإدارة التراخيص"}</span>
                </button>
              </form>
            )}

            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-xs text-amber-300 space-y-1 text-right">
              <p className="font-bold flex items-center gap-1.5 text-amber-400">
                <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />
                تنبيه هام من إدارة النظام (إتقان سوفت):
              </p>
              <p className="text-slate-300 leading-relaxed text-[11px]">
                تم قفل النظام لحماية التراخيص. لا يمكن للمستخدمين استخدام الشاشات إلا بعد قيام مطور/مالك النظام بتسجيل الدخول واعتماد ترخيص جديد للجهاز.
              </p>
            </div>
