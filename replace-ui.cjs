...
// Remove QuickAirline Modal
code = code.replace(/\{\/\* Quick Airline Modal \*\/\}.*?<\/Dialog>/gs, '');

// Update the Form Layout UI
const newFormUI = `
...
// Replacing everything from `<div className="grid grid-cols-1 md:grid-cols-2 gap-4">` down to `</div>              {/* Commission Live Display */}`
const startString = `<div className="grid grid-cols-1 md:grid-cols-2 gap-4">`;
...
if (startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + newFormUI + code.substring(endString);
}
