...
const generateTrialStart = 'export function generateTrialBalanceA4Html';
const generateTrialEnd = 'export function generateFinancialA4Html';

const regex = new RegExp(`(${generateTrialStart})[\\s\\S]*?(?=${generateTrialEnd})`);

const newGenerateTrial = `export function generateTrialBalanceA4Html({
  accounts, startDate, endDate, settings
}: any) {
  ...
  const rowsHtml = accounts.map((acc:any) => {
    // Generate some mock values for missing trial balance data
  ...
  <div style="font-family: 'Tajawal', sans-serif; max-width: 100%; margin: 0 auto;">
    ...
    <div style="font-size: 14px; margin-top: 5px;">0</div>
...
code = code.replace(regex, newGenerateTrial);
fs.writeFileSync('artifacts/pos-system/src/lib/printUtils.ts', code);
console.log('Replaced generateTrialBalanceA4Html');
