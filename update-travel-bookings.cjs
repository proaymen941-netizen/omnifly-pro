...
// 1. Replace the airline fetch endpoint
code = code.replace(
  'fetchWithAuth("/api/travel/airlines")',
  'fetchWithAuth("/api/travel/sub-accounts/21100")'
);

// 2. Remove quickAirlineModal stuff entirely
code = code.replace(/const \[quickAirlineModal, setQuickAirlineModal\] = useState\(false\);[\s\S]*?const addAirlineMutation = useMutation\(\{[\s\S]*?\}\);/, '');

// 3. Form State replacement
code = code.replace(/const \[form, setForm\] = useState\(\{[\s\S]*?\}\);/, `const [form, setForm] = useState({
    booking_number: "",
    service_type: "flight",
    customer_id: "",
    passenger_id: "",
...
// 4. resetForm replacement
code = code.replace(/const resetForm = \(\) => \{[\s\S]*?setForm\(\{[\s\S]*?\}\);\s*\};/, `const resetForm = () => {
...
// 5. handleEdit replacement
code = code.replace(/const handleEdit = \(bk: any\) => \{[\s\S]*?setForm\(\{[\s\S]*?\}\);\s*setModalOpen\(true\);\s*\};/, `const handleEdit = (bk: any) => {
...
// 6. Delete Quick Airline Modal component
code = code.replace(/\{\/\* Quick Airline Modal \*\/\}[\s\S]*?<\/Dialog>/, '');
...
const startString = `<div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700">العميل (Customer) *</label>`;
const endString = `{/* Commission Live Display */}`;

const startIndex = code.indexOf(startString);
const endIndex = code.indexOf(endString);

if (startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + newFormUI + '\n              ' + code.substring(endString);
} else {
